...
public class Tracking_Test : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Chooses which target the enemy will aproach depending on if the player is in the same room as the player
        if(findPlayer == true)
        {
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, playerPos.playerPosition, moveSpeed * Time.deltaTime);
        }
    }
